...
export default function AdminScanner() {
  ...
  // 3. Send cropped image to Gemini + ImgBB
  const handleScan = async () => {
    ...
    try {
      ...
      // Pass the cropped blob instead of the original file
      formData.append('image', croppedImageBlob, 'card.jpg');
      ...
      <div className="max-w-2xl mx-auto bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            >
              <div className="text-slate-500 flex flex-col items-center">
                ...
                <span className="font-medium tracking-wide text-center px-4">Tap to Take Photo or Upload</span>
              </div>
            </label>
          </div>
        )}
        ...
        {imageSrc && !croppedImageBlob && (
          <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-300">
            <p className="text-amber-500 font-bold uppercase tracking-wider text-sm">Pinch & Drag to Frame Card</p>
            ...
            <div className="relative w-full aspect-[3/4] max-w-sm rounded-2xl overflow-hidden bg-slate-950 border border-slate-700">
              <Cropper
                ...
                crop={crop}
                zoom={zoom}
                aspect={3 / 4} // Trading card aspect ratio
            ...
            <button 
              ...
              className="w-full max-w-sm py-4 rounded-xl font-bold text-lg bg-emerald-500 hover:bg-emerald-400 text-slate-950 transition-all flex items-center justify-center gap-2"
            >
              Confirm Crop
        ...
        {/* STEP 3: Preview Cropped & Analyze */}
        {croppedImageBlob && !scannedData && (
          <div className="flex flex-col items-center gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <p className="text-emerald-500 font-bold uppercase tracking-wider text-sm">Perfect Crop!</p>
            <div className="w-full aspect-[3/4] max-w-sm rounded-2xl overflow-hidden border-2 border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.3)]">
            ...
            <button 
              onClick={handleScan}
              disabled={scanning}
              className="w-full max-w-sm py-4 rounded-xl font-bold text-lg bg-amber-500 hover:bg-amber-400 text-slate-950 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(245,158,11,0.2)]"
            >
              {scanning ? (
                <>
            ...
            <button 
              ...
              disabled={scanning}
              className="text-slate-400 hover:text-white text-sm underline disabled:opacity-50"
            >
              Re-Crop Image
            </button>
          </div>
          ...
          <form onSubmit={handleSubmitToSheet} className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex gap-4 mt-8 pt-6 border-t border-slate-800">
              ...
              <button 
                type="submit"
                disabled={submitting}
                className="flex-[2] py-4 rounded-xl font-bold text-lg bg-emerald-500 hover:bg-emerald-400 text-slate-900 transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)] hover:shadow-[0_0_30px_rgba(16,185,129,0.4)] disabled:opacity-50 flex items-center justify-center gap-2"
              >
